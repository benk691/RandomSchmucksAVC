...
class SimControlPlanner(Thread):
  '''
  ...
  def __init__(self, group=None, target=None, name=None, args=(), kwargs={}, daemon=None, vehicle=None, particleFilter=None, courseMap=None):
    ...
    '''
    super(SimControlPlanner, self).__init__(group=group, target=target, name=name, daemon=daemon)
  ...
  def _control(self):
    ...
    self.steeringAngleGoal = math.atan(Constants.VEHICLE_AXLE_LEN * ((2.0 * yErr) / (math.pow(xErr, 2) + math.pow(yErr, 2))))
